...
if($user['lvl']<10){
header('Location: /duel/index.php?error=1&bot=gold');
exit;
}
...
$req = mysql_query("SELECT * FROM `bots_gold` WHERE `user_id` = '".$user['id']."'");
$bot = mysql_fetch_array($req);
if($bot['attacked']==1){
header('Location: /duel/bot_gold.php?');
end;
}
...
$powermin=$user['power']+15;
$powermax=$user['power']+20;
$botpower=rand($powermin, $powermax);
$defensemin=$user['defense']+15;
$defensemax=$user['defense']+20;
$botdefense=rand($defensemin, $defensemax);
$speedmin=$user['speed']+15;
$speedmax=$user['speed']+20;
$botspeed=rand($speedmin, $speedmax);
$skillmin=$user['skill']+15;
$skillmax=$user['skill']+20;
$botskill=rand($skillmin, $skillmax);
$healthmin=$user['health']+15;
$healthmax=$user['health']+20;
$bothealth=rand($healthmin, $healthmax);

//Если не создана таблица ботов для игрока
if(empty($bot)){
mysql_query("INSERT INTO `bots_gold` SET 
		`user_id` = '".$user['id']."'");
}

//Обновляем бота в таблице
mysql_query("UPDATE `bots_gold` SET
 `power` = '$botpower',
 `defense` = '$botdefense',
 `speed` = '$botspeed',
 `skill` = '$botskill',
...
<b>'.$user['login'].'</b> против <b>Золотой Наёмник</b></center>';
...
<td align="center" width="50%" class="spacing">
<img src="/images/faces/bots/bot_gold.png"><br></a></td>
</tr></table>';
...
<li><a href="/duel/bot_gold.php?"><img src="/images/icons/sword.png">Следующий</a></li>
...
$userhit=0;
$userspeedwhy=' (Золотой Наёмник увернулся)';
}
...
//Награда серебра
$maxsilver=$gameset['bot_gold_silver']*$user['lvl'];
...
//Награда золота
$maxgold=$gameset['bot_gold_gold']*$user['lvl'];
$mingold=$maxgold/4;
$goldrand=rand($mingold, $maxgold);
$newgold=$user['gold']+$goldrand;
mysql_query("UPDATE `users` SET
 `exp` = '$newexp',
 `silver` = '$newsilver',
 `gold` = '$newgold',
...
Награда: <img src="/images/icons/gold.png">'.n_f($goldrand).' золота<hr>
<b>Нанесённый урон:</b><br/>
'.$user['login'].': '.$userhit.''.$userskillwhy.'<br/>
Золотой Наёмник: '.$bothit.''.$botspeedwhy.'<hr>';
...
Золотой Наёмник: '.$bothit.''.$botskillwhy.'<hr>';
